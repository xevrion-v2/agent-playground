/**
 * High-precision PI calculation using Chudnovsky algorithm with binary splitting.
 *
 * The Chudnovsky algorithm converges at ~14 digits per term,
 * making it the fastest known algorithm for PI computation.
 *
 * Formula:
 ...
 * @returns PI formatted with the requested decimal precision
 */
 ...
 * @returns PI scaled as a BigInt
 */
export function calculatePiScaled(decimalPlaces: number): bigint {
  if (!Number.isSafeInteger(decimalPlaces) || decimalPlaces < 0) {
    throw new TypeError("decimalPlaces must be a non-negative safe integer");
  }
 ...
 * Returns [p, q, t] where the series sum is q/t.
 */
function binarySplit(start: number, end: number): [bigint, bigint, bigint] {
  if (end - start === 1) {
    if (start === 0) {
...
/**
 * Known 100-digit value of PI for verification.
 */
